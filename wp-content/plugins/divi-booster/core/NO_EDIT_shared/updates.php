<?php
namespace DiviBooster\DiviBooster\Updater;
...
try {
    new DBAL_PluginUpdateChecker(
        $config['update_url'].'/updates.json',
        $config['plugin_file'],
        $config['plugin_slug']
    );
} catch (\Exception $e) {} 
 ...
 * @access public
 */
class DBAL_PluginUpdateChecker {
	...
	public function requestInfo($queryArgs = array()){
		...
		if ( !is_wp_error($result) && isset($result['response']['code']) && ($result['response']['code'] == 200) && !empty($result['body']) ){
			$pluginInfo = DBAL_PluginInfo::fromJson($result['body'], $this->debugMode);
	...
	public function requestUpdate(){
		...
		return DBAL_PluginUpdate::fromPluginInfo($pluginInfo);
	}
	...
	public function getUpdateState() {
		...
		if ( !empty($state) && isset($state->update) && is_object($state->update) ){
			$state->update = DBAL_PluginUpdate::fromObject($state->update);
		}
		return $state;
	}
	...
	public function displayManualCheckResult() {
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			if ($last_error) { 
				printf('<div class="error"><p>%s</p></div>', htmlentities($last_error, ENT_QUOTES, "UTF-8"));
			...
			if ( $status == 'no_update' ) {
				...
				$message = sprintf('Unknown update checker status "%s"', htmlentities($status, ENT_QUOTES, "UTF-8"));
			}
			printf(
	...
	protected function isMuPlugin() {
		...
		if ($cachedResult === null && defined('WPMU_PLUGIN_DIR')) {
			//Convert both paths to the canonical form before comparison.
			$muPluginDir = realpath(WPMU_PLUGIN_DIR);
			$pluginPath  = realpath($this->pluginAbsolutePath);
			if (is_string($muPluginDir)) {
...
class DBAL_PluginUpdate {
	...
	public static function fromJson($json, $triggerErrors = false){
		...
		$pluginInfo = DBAL_PluginInfo::fromJson($json, $triggerErrors);
		if ( $pluginInfo != null ) {
